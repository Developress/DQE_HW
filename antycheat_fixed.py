...
def set_words(word_len: int, filename: str = "C:\\DB\\words.txt") -> typing.Set[str]:
    """
...
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int,
              likeliest_letter: str) -> typing.Tuple[str, int]:
    """
        Исправляет ввод пользователя на основе предыдущих букв и длинны слова.

        :param likeliest_letter: letter computer offered
    ...
    another_letter_entered: bool = likeliest_letter not in user_input

    while another_letter_entered:
        print("Something went wrong.")
        print("The computer has offered another letter")
        print(f"It was {likeliest_letter}")
        corrected_input = input("Try again   ").lower()

        another_letter_entered = likeliest_letter not in corrected_input
...
#Инициализируем все перед началом игры
    ...
    likeliest_letter: typing.Tuple[str, float] = "_", 0
    ...
    while is_playing:
        if was_correct:
            ...
            current_word, len_of_word = antycheat(current_word, last_word, len_of_word, likeliest_letter[0])
        ...
        likeliest_letter = get_percent(stats)
